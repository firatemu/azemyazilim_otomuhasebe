            ...
            try {
                switch (name) {
                    case 'container_status':
                        return await this.containerStatus(args?.filter || 'running');
                    case 'quick_restart':
                        return await this.quickRestart(args?.service);
                    case 'stream_logs':
                        return await this.streamLogs(args?.service, args?.lines || 50);
                    case 'resource_usage':
...
- **Portlar:** ${this.formatPorts(container.Ports)}
    ...
    async quickRestart(service) {
        const containerName = SERVICE_NAMES[service];
        if (!containerName) {
    ...
    async streamLogs(service, lines) {
        const containerName = SERVICE_NAMES[service];
        if (!containerName) {
            throw new Error(`Geçersiz servis: ${service}`);
        }
