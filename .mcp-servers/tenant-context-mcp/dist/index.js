    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    case 'list_tenants':
                        return await this.listTenants(args?.activeOnly ?? true);
                    case 'tenant_details':
                        return await this.tenantDetails(args?.tenantId);
                    case 'tenant_data_count':
                        return await this.tenantDataCount(args?.tenantId);
                    case 'test_isolation':
                        return await this.testIsolation(args?.tenantId);
                    case 'tenant_activity':
                        return await this.tenantActivity(args?.tenantId, args?.limit || 10);
                    default:
...
`).join('\n')}
    ...
    async tenantDetails(tenantId) {
        ...
        try {
            ...
            const result = await client.query('SELECT * FROM tenants WHERE id = $1 OR subdomain = $1', [tenantId]);
            if (result.rows.length === 0) {
...
- **Vergi Dairesi:** ${tenant.vergiDairesi || 'Belirtilmemiş'}
    ...
    async tenantDataCount(tenantId) {
        const client = await this.getClient();
        try {
            // Tenant'ın varlığını kontrol et
            const tenantCheck = await client.query('SELECT id, "firmaAdi" FROM tenants WHERE id = $1', [tenantId]);
...
**Toplam Veri:** ${counts.reduce((sum, c) => sum + parseInt(c.rows[0].count), 0)} kayıt
    ...
    async testIsolation(tenantId) {
        ...
        try {
            ...
            const otherTenantData = await client.query('SELECT COUNT(*) FROM stoklar WHERE "tenantId" != $1', [tenantId]);
            const ownData = await client.query('SELECT COUNT(*) FROM stoklar WHERE "tenantId" = $1', [tenantId]);
            const isolationTest = `
...
- API endpoint'lerinde tenant doğrulaması yapın
    ...
    async tenantActivity(tenantId, limit) {
        const client = await this.getClient();
        try {
            // Tenant'ın varlığını kontrol et
            const tenantCheck = await client.query('SELECT id, "firmaAdi" FROM tenants WHERE id = $1', [tenantId]);
