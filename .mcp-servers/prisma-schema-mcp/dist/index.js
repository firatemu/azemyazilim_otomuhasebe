...
class PrismaSchemaServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    case 'analyze_schema':
                        return await this.analyzeSchema(args?.environment || 'staging');
                    case 'show_relations':
                        return await this.showRelations(args?.modelName, args?.environment || 'staging');
                    case 'suggest_indexes':
                        return await this.suggestIndexes(args?.environment || 'staging');
                    case 'list_models':
                        return await this.listModels(args?.environment || 'staging');
                    default:
    ...
    async readSchema(environment) {
        const schemaPath = SCHEMA_PATHS[environment];
        if (!schemaPath) {
            throw new Error(`Geçersiz ortam: ${environment}`);
        }
        return await readFile(schemaPath, 'utf-8');
    }
    async analyzeSchema(environment) {
...
${enums.map((e) => `- ${e.name} (${e.values.length} değer)`).join('\n')}
    ...
    async showRelations(modelName, environment) {
        const schema = await this.readSchema(environment);
        const dmmf = await getDMMF({ datamodel: schema });
...
- **Liste:** ${rel.isList ? 'Evet' : 'Hayır'}
    ...
    async suggestIndexes(environment) {
        const schema = await this.readSchema(environment);
        const dmmf = await getDMMF({ datamodel: schema });
        const suggestions = [];
        dmmf.datamodel.models.forEach((model) => {
            // Foreign key alanlarını kontrol et
            const foreignKeys = model.fields.filter((f) => f.relationName && !f.isList);
            foreignKeys.forEach((fk) => {
                const hasIndex = model.fields.some((f) => f.name === fk.name && f.isId);
                if (!hasIndex) {
                    suggestions.push(`${model.name}.${fk.name} - Foreign key için index eklenebilir`);
                }
            });
            ...
            commonFilterFields.forEach((fieldName) => {
                ...
                if (field) {
                    suggestions.push(`${model.name}.${fieldName} - Sık filtreleme için index önerilir`);
                }
            });
        });
