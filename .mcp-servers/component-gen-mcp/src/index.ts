...
class ComponentGenServer {
            ...
            try {
                switch (name) {
                    case 'generate_crud_page':
                        return await this.generateCrudPage(
                            args?.modelName as string,
                            args?.fields as string
                        );
                    case 'generate_form':
                        return await this.generateForm(
                            args?.formName as string,
                            args?.fields as string
                        );
                    case 'generate_hooks':
                        return await this.generateHooks(
                            args?.modelName as string,
    ...
    private async generateCrudPage(modelName: string, fieldsJson: string) {
...
\`\`\`
    ...
    private async generateForm(formName: string, fieldsJson: string) {
        const fields: Field[] = JSON.parse(fieldsJson);
...
\`\`\`
    ...
    private async generateHooks(modelName: string, endpoint: string) {
        const lowerModel = modelName.toLowerCase();
...
\`\`\`
    ...
    private async generateTypes(modelName: string, fieldsJson: string) {
        const fields: Field[] = JSON.parse(fieldsJson);
