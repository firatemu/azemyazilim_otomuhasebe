...
class ApiDocsServer {
            ...
            try {
                switch (name) {
                    case 'discover_endpoints':
                        return await this.discoverEndpoints(args?.environment || 'staging');
                    case 'test_endpoint':
                        return await this.testEndpoint(args?.method, args?.path, args?.body || '', args?.environment || 'staging');
                    case 'health_check_all':
                        return await this.healthCheckAll(args?.environment || 'staging');
                    case 'generate_postman':
                        return await this.generatePostman(args?.environment || 'staging');
                    default:
    ...
    async discoverEndpoints(environment) {
...
`).join('\n')}
    ...
    async testEndpoint(method, path, body, environment) {
...
${error.response?.data ? `\`\`\`json\n${JSON.stringify(error.response.data, null, 2)}\n\`\`\`` : ''}
    ...
    async healthCheckAll(environment) {
        const baseUrl = API_URLS[environment];
        ...
        const results = await Promise.all(criticalEndpoints.map(async (endpoint) => {
            try {
                const startTime = Date.now();
                await axios({
...
**Özet:** ${results.filter(r => r.status === 'success').length}/${results.length} endpoint sağlıklı
    ...
    async generatePostman(environment) {
        const baseUrl = API_URLS[environment];
        const collection = {
            info: {
