...
class DatabaseQueryServer {
                ...
                switch (name) {
                    ...
                    case 'analyze_slow_queries':
                        return await this.analyzeSlowQueries(
                            ...
                            (args?.minDuration as number) || 1000
                        );
                    case 'explain_query':
    ...
    private async getClient(environment: string): Promise<Client> {
        const config = DB_CONFIGS[environment as keyof typeof DB_CONFIGS];
        if (!config) {
    ...
    private async sqlToPrisma(sql: string, tableName: string) {
...
> **Not:** Bu otomatik çeviridir. WHERE koşullarını ve diğer detayları manuel olarak düzenlemeniz gerekebilir.
    ...
    private async analyzeSlowQueries(environment: string, minDuration: number) {
...
`).join('\n')}
    ...
    private async explainQuery(sql: string, environment: string) {
        const client = await this.getClient(environment);
...
\`\`\`
    ...
    private async backupDatabase(environment: string) {
        const config = DB_CONFIGS[environment as keyof typeof DB_CONFIGS];
        ...
        try {
            const { stdout, stderr } = await execAsync(
                `docker exec otomuhasebe-postgres pg_dump -U ${config.user} ${config.database} > ${backupFile}`
            );
    ...
    private async tableStats(tableName: string, environment: string) {
        const client = await this.getClient(environment);
