...
export default function AlisIadeFaturalariPage() {
  ...
  const [formData, setFormData] = useState({
    invoiceNo: '',
    invoiceType: 'ALIS_IADE' as 'SATIS_IADE' | 'ALIS_IADE',
    accountId: '',
  ...
  const handleExportExcel = async () => {
    try {
      const params: Record<string, string> = { type: 'ALIS_IADE' };
  ...
  const openAddDialog = async () => {
    resetForm();
    try {
      const response = await axios.get('/code-templates/preview-code/INVOICE_PURCHASE_RETURN');
      if (response.data?.nextCode) {
        ...
        setFormData(prev => ({
          ...prev,
          invoiceNo: `AIF-${new Date().getFullYear()}-${newNo}`,
        }));
      }
    } catch (error: any) {
      const lastFatura = faturalar[0];
  ...
  const fetchStats = async () => {
    try {
      const params: Record<string, any> = { type: 'ALIS_IADE' };
      if (filterStartDate) params.startDate = filterStartDate;
      if (filterEndDate) params.endDate = filterEndDate;
      if (filterDurum.length > 0) params.status = filterDurum.join(',');
      if (filterCariId) params.accountId = filterCariId;
    ...
    return (
      ...
      >
        <DialogTitle component="div" sx={{ fontWeight: 'bold' }}>
          {openAdd ? 'Yeni Alış İade Faturası' : 'Alış İade Faturası Düzenle'}
        </DialogTitle>
        <DialogContent>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                ...
                <Button
                  ...
                  sx={{
                    ...
                    fontWeight: 600,
                    '&:hover': {
                      bgcolor: 'color-mix(in srgb, var(--chart-2) 85%, var(--background))',
                    },
                  }}
                >
                  Kalem Ekle
  ...
  const handleEdit = (row: Fatura) => {
    const tabId = `invoice-return-purchase-edit-${row.id}`;
    addTab({
      id: tabId,
    ...
    },
    {
      ...
      getActions: (params) => [
        ...
        <Tooltip title="Yazdır">
          <IconButton size="small" onClick={(e) => { e.stopPropagation(); window.open(`/fatura/iade/alis/print/${params.row.id}`, '_blank'); }}>
  ...
  return (
    <StandardPage maxWidth={false}>
      ...
      <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1.5 }}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
          <Button
            ...
            onClick={() => {
              addTab({
                id: 'invoice-return-purchase-yeni',
                label: 'Yeni Alış İade Faturası',
                path: '/invoice/return/purchase/yeni'
              });
              setActiveTab('invoice-return-purchase-yeni');
      ...
      {/* KPI Kartları */}
      <KPIHeader loading={loading} data={kpiData} type="ALIS_IADE" />
      ...
      <Dialog open={openIptal} onClose={() => { setOpenIptal(false); setIrsaliyeIptal(false); }} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
        <DialogTitle component="div" sx={{
          background: 'linear-gradient(135deg, var(--chart-2) 0%, var(--chart-2) 100%)',
      ...
      >
        {(() => {
          ...
          return [
            ...
            <MenuItem key="print" onClick={() => { handleMenuClose(); window.open(`/fatura/iade/alis/print/${fatura.id}`, '_blank'); }}>
            ...
            <MenuItem
              ...
              onClick={() => {
                handleMenuClose();
                const path = `/invoice/return/purchase/yeni?kopyala=${fatura.id}`;
                const tabId = `fatura-iade-alis-kopyala-${fatura.id}`;
